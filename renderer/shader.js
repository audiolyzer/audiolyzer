...
//				classpath.construct(vsh, fsh);
	...
	construct(vsh, fsh) {
		...
		if(!gl.getShaderParameter(vertex, gl.COMPILE_STATUS)) {
			console.error("Could not compile vertex shader! " + gl.getShaderInfoLog(vertex));
			return;
		}
		gl.compileShader(fragment);
		if(!gl.getShaderParameter(fragment, gl.COMPILE_STATUS)) {
			console.error("Could not compile fragment shader! " + gl.getShaderInfoLog(fragment));
			return;
		}
		...
		if(!gl.getProgramParameter(this.program, gl.LINK_STATUS)) {
			console.error("Could not link shader program! " + gl.getProgramInfoLog(this.program));
			return;
		}
		gl.validateProgram(this.program);
		if(!gl.getProgramParameter(this.program, gl.VALIDATE_STATUS)) {
			console.error("Could not validate shader program! " + gl.getProgramInfoLog(this.program));
			return;
		}
